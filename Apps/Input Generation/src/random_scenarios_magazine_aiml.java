...
/*
 * Script for generating the random scenarios used in AIML & Wi-Fi magazine paper
...
public class random_scenarios_magazine_aiml {
    ...
    public static void generate_wlans(Point2D.Double[] aps_position_list, Point2D.Double[][] stas_position_list, int obss_pd_val) {
        ...
        for (int w = 0; w < num_wlans; w++) {
            ...
            if (obss_pd_val == -82) {
                wlan_aux.bss_color = -1; // TO DISABLE SR
            } else {
                wlan_aux.bss_color = wlan_id + 1;
            }            
            wlan_aux.spatial_reuse_group = 0;
            wlan_aux.non_srg_obss_pd = obss_pd_val;
    ...
    public static void generate_file(String csv_filepath_output)
        ...
        // NEW PARAMETERS FOR DAVID (TO ADAPT TO CURRENT FORMAT)
        int traffic_model_default = 99;
        int packet_length_default = 12000;
        int num_aggregated_default = 64;
        int capture_effect_model_default = 0;
        int capture_effect_thr_default = 10;
        int constant_per_default = 0;
        int pifs_activated_default = 0; 
        int cont_wind_activated = 1;
        ...
        String csv_header_line = "node_code" + CSV_SEPARATOR
                ...
                + "capture_effect_threshold" + CSV_SEPARATOR
        ...
        for (int w = 0; w < num_wlans; w++) {
            ...
            line = getCompleteLine(wlan.ap_code, node_type,
                ...
                traffic_model_default, packet_length_default, num_aggregated_default,
                capture_effect_model_default, capture_effect_thr_default, constant_per_default,
                pifs_activated_default, cont_wind_activated, wlan.bss_color, wlan.spatial_reuse_group, 
    ...
    static String getCompleteLine(String node_code, int node_type,
            ...
            int tpc_default, int cca_default, int traffic_model_default, int packet_length_default, int num_aggregated_default,
                int capture_effect_model_default, int capture_effect_thr_default, int constant_per_default,
                int pifs_activated_default, int cont_wind_activated, int bss_color, int spatial_reuse_group, 
        ...
        String line = node_code + CSV_SEPARATOR
                ...
                + traffic_model_default + CSV_SEPARATOR
                + traffic_load + CSV_SEPARATOR
                + packet_length_default + CSV_SEPARATOR
                + num_aggregated_default + CSV_SEPARATOR
                + capture_effect_model_default + CSV_SEPARATOR
                + capture_effect_thr_default + CSV_SEPARATOR
                + constant_per_default + CSV_SEPARATOR // CW
                + pifs_activated_default + CSV_SEPARATOR // CW's max stage                             
                + cont_wind_activated + CSV_SEPARATOR
    ...
    public static void main(String args[]) throws IOException {
        ...
        String input_path = "./input_constructor/input_template_david.csv";
        ...
        for (int n = 0 ; n < num_scenarios; n ++) {    
            ...
            for (int i = 0; i < num_wlans; i++) {      
                for (int j = 0; j < num_stas_per_wlan; j++) {
                    ...
                    */
                    ...
                    System.out.println("Dmin: " + d_min_AP_STA + " / Dmax: " + d_max_AP_STA);
            ...
            for (int out_ix1 = 0; out_ix1 < obss_pd_list.length; out_ix1++) {
            
                // GENERATE EACH .CSV FILE
                for (int out_ix = 1; out_ix <= num_outputs; out_ix++) {
                    generate_wlans(aps_position_list, stas_position_list, obss_pd_list[out_ix1]);
                    output_path = "./output/input_nodes_sce" + n + "_cca" + obss_pd_list[out_ix1] + ".csv";
