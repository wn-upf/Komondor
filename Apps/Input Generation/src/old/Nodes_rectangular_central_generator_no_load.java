 ...
 */
public class Nodes_rectangular_central_generator_no_load {

    static Wlan[] wlan_container = null;
    static int num_stas_wlan_a_min;
    static int num_stas_wlan_a_max;
    ...
    public static void generate_wlans() {
        ...
        for (int w = 0; w < num_wlans; w++) {
            ...
            if (w == 0) {
                ...
                int num_stas_wlan_a = random.nextInt(num_stas_wlan_a_max) + num_stas_wlan_a_min;
                list_sta_code = new String[num_stas_wlan_a];
                for (int n = 0; n < num_stas_wlan_a; n++) {
                    list_sta_code[n] = "STA_" + wlan_code + "" + (n + 1);
                }

                wlan_aux = new Wlan(wlan_id, wlan_code, num_stas_wlan_a, ap_code,
                        list_sta_code, primary_channel, min_ch_allowed,
                        max_ch_allowed, wlan_80211ax, x, y, z, channel_bonding_aux, 0);
...
//        Map_viewer.main(map_viewer_args);
        ...
        // String input_path = args[0];
        num_stas_wlan_a_min = 1;
        num_stas_wlan_a_max = 20;
        ...
        for (int out_ix = 1; out_ix <= num_outputs; out_ix++) {
            ...
            for (int w = 1; w < num_wlans; w++) {
                ...
                wlan_container[w].traffic_load = 0;

            }

            wlan_container[0].traffic_load = 0;
            ...
            output_path = "input_nodes_n" + num_wlans + "_s" + String.format("%03d", out_ix) + "_cb" + channel_bonding_model
                    + ".csv";
            System.out.println("output_path: " + output_path);
            genearate_file(output_path);
