 ...
 * @author UPF
 */
public class ThreeWLANsGenerator {
    ...
    public static void genearate_file(String csv_filepath_output,
            int channel_bonding_model, double traffic_load_A, double traffic_load_B)
        ...
        wlan_B.y = 0;
        wlan_B.z = 0;
        wlan_B.primary_channel = 2;
        wlan_B.min_ch_allowed = 2;
        wlan_B.max_ch_allowed = 3;
        ...
        // AP_C
        Wlan wlan_C = new Wlan();
        wlan_C.ap_code = "AP_C";
        wlan_C.wlan_code = "C";
        wlan_C.x = 80;
        wlan_C.y = 0;
        wlan_C.z = 0;
        wlan_C.primary_channel = 0;
        wlan_C.min_ch_allowed = 0;
        wlan_C.max_ch_allowed = 1;
        wlan_C.channel_bonding_model = channel_bonding_model;
        wlan_C.traffic_load = traffic_load_A;

        line = getCompleteLine(wlan_C.ap_code, 0, wlan_C.wlan_code, wlan_C.x,
                wlan_C.y, wlan_C.z, wlan_C.primary_channel, wlan_C.min_ch_allowed,
                wlan_C.max_ch_allowed, wlan_C.channel_bonding_model, wlan_C.traffic_load);

        out.println(line);

        // STA_C1
        line = getCompleteLine("STA_C1", 1, wlan_C.wlan_code, wlan_C.x,
                wlan_C.y + 1, wlan_C.z, wlan_C.primary_channel,
                wlan_C.min_ch_allowed, wlan_C.max_ch_allowed,
                wlan_C.channel_bonding_model, wlan_C.traffic_load);
    ...
    public static void main(String args[]) throws IOException {
        ...
        double[] traffic_loads_A = {1, 50, 100, 150, 200, 250, 300, 350, 400, 450, 500, 550, 600, 650, 700, 750, 800, 850, 900, 950, 1000};
        double[] traffic_loads_B = {100, 250, 400};
        
        // AM
        int ch_allocation_id = 1;
        int ch_bonding_model = 4;
        for (int loadB_ix = 0; loadB_ix < traffic_loads_B.length; loadB_ix++) {

            for (int loadA_ix = 0; loadA_ix < traffic_loads_A.length; loadA_ix++) {

                output_path = "input_nodes_3overlapWLANs_AM_ca" + ch_allocation_id
                        + "_loadB_" + String.format("%03d", (int) traffic_loads_B[loadB_ix])
                        + "_loadAandC_" + String.format("%03d", (int) traffic_loads_A[loadA_ix]) + ".csv";
                genearate_file(output_path, ch_bonding_model, traffic_loads_A[loadA_ix], traffic_loads_B[loadB_ix]);
            }
