 ...
 *      + BSS Coloring for OBSS_PD-based SR 
 *      + Tune only the OBSS_PD threshold
 *      + Use the minimum tx power (1 dBm)
 */
/**
 *
 * @author Sergio Barrachina-Munoz (sergio.barrachina@upf.edu)
 */
public class SR_RANDOM_1_bounded {
    ...
    public static void generate_wlans(int obss_pd, Point2D.Double[] aps_position_list, 
            Point2D.Double[] stas_position_list, int traffic_load) {
        
//        int size = aps_position_list.length;
//        System.out.println("-----------------------");
//        for (int i = 0 ; i < size; i ++) {
//            System.out.println("aps_position_list[" + i + "] = " + aps_position_list[i]);
//        }
    ...
    public static void main(String args[]) throws IOException {
        ...
        String input_template = "input_template_random_" + type_of_scenario;
        String input_path = "./input_constructor/random_scenarios/" + input_template + ".csv";
        System.out.println("input_path: " + input_path);
        String output_path = "./output/*";
        String beginning_output_path = "./output/*";
        ...
        // DEFINE THE TRAFFIC LOADS ALLOWED
        int[] traffic_loads = {1000, 5000, 10000};
...
//                        x = (map_width/3) * (i-1)%3 * new Random().nextDouble();
//                        y = (map_heigth/3) * (3+i-1)%3 * new Random().nextDouble();
                    }
                    if (i > 0) {
                        for (int j = 0; j < i; j++) {
            ...
            for (int out_ix = 1; out_ix <= num_outputs; out_ix++) {
                for (int j = 0 ; j < traffic_loads.length; j ++) {
                    beginning_output_path = "./output/traffic_load_"+traffic_loads[j];
                    for (int f = 0; f < obss_pd_list.length; f++) {  
                        generate_wlans(obss_pd_list[f], aps_position_list, stas_position_list, traffic_loads[j]);
                        output_path = beginning_output_path + "/input_nodes_" + type_of_scenario + "_sce" + n + 
