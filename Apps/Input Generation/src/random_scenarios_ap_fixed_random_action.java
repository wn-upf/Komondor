...
public class random_scenarios_ap_fixed_random_action {
    ...
    public static void main(String args[]) throws IOException {
        ...
        int num_rows = 3;
        int aux_count = 0;
        double[][] grid_centers = new double[num_wlans][2];   
        for (int c = 0; c < num_wlans; c++) {
            grid_centers[c][0] = (c%num_cols)*(map_width/num_cols) + (map_width/num_cols)/2;
            grid_centers[c][1] = (map_heigth/num_rows)/2 + aux_count*(map_heigth/num_rows);
            if (c%num_rows == num_rows-1){ aux_count++;}
        }               
        ...
        for (int n = 0 ; n < num_scenarios; n ++) {        
            ...
            for (int i = 0; i < num_wlans; i++) {
                ...
                // Assign the random poin to the node's location
                Point2D.Double point = new Point2D.Double();
                point.setLocation(x, y);
