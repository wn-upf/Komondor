...
public class only_same_channel_enterprise_scenario_11ax1 {
    ...
    static Point2D.Double[] aps_position_list;
    static Point2D.Double[][] stas_position_list;
...
//            System.out.println("Setting WLAN " + (w+1) + "/" + num_wlans);
...
//            primary_channel = ThreadLocalRandom.current().nextInt(0, c_sys_width);
            ...
            wlan_aux = new Wlan(wlan_id, wlan_code, num_stas, ap_code,
                ...
                max_ch_allowed, wlan_80211ax, aps_position_list[w].x, aps_position_list[w].y, 
                3, channel_bonding_model, traffic_load);                   
            ...
            wlan_aux.spatial_reuse_group = 0;
    ...
    public static void generate_file(String csv_filepath_output)
        ...
        for (int w = 0; w < num_wlans; w++) {
            ...
            line = getCompleteLine(wlan.ap_code, node_type,
                wlan.wlan_code, aps_position_list[w].x, aps_position_list[w].y, 
                3, wlan.primary_channel, wlan.min_ch_allowed, wlan.max_ch_allowed,
...
//             System.out.println(line);
            ...
            // Set STAs location
            for (int n = 0; n < wlan.num_stas / num_wlans; n++) {                                
                line = getCompleteLine(wlan.list_sta_code[n], node_type,
                    wlan.wlan_code, stas_position_list[w][n].x, 
                    stas_position_list[w][n].y, 1, wlan.primary_channel,
...
//                    System.out.println(line);
    ...
    public static void DefineNodesLocations() 
        throws UnsupportedEncodingException, FileNotFoundException, IOException {
        
        // Number of offices distributed in rows and columns
        int num_rows = 2;
        int num_columns = 4;
        
        // Number of APs in each office
        int number_of_aps_per_office = 1;
                
        // Use this to locate each of the 4 APs in each office
        int[][] ap_shifts = {{-15,-15},{-15,-5},{-5,-15},{-5,-5}};
        
        // Ceils
        int ceil_height = 4;
        int ceil_width = 4;
        double distance_ceils;
        int[][] ceil_shifts_local = {{-3,-3},{-3,-1},{-1,-3},{-1,-1}};
        double[][] ceil_shifts_inter = {{-2.5,-2.5},{2.5,-2.5},{-2.5,2.5},{2.5,2.5}};
        double[][] cubicle_shifts = {{-1,-1},{1,-1},{-1,1},{1,1}};
        
        // AP locations
        int ap_ix = 0;
        for (int i = 0; i < num_rows; i ++) {
            for (int j = 0; j < num_columns; j ++) {                
                for (int a = 0; a < number_of_aps_per_office; a ++) {
                    // Define location of the AP                    
                    Point2D.Double point = new Point2D.Double();
                    point.setLocation((map_heigth/2)*(j%num_columns) + (map_heigth/2) + ap_shifts[a][1], 
                        (map_width/4)*i + (map_width/4) + ap_shifts[a][0]); 
                    aps_position_list[ap_ix] = point;
//                    System.out.println("AP" + ap_ix + "'s position = " + aps_position_list[ap_ix]);
                    // Define cubicles (4 per AP)
                    int sta_ix = 0;
                    for(int k = 0; k < 4; k ++) {
                        // Define the shift between ceils (different from ceil to ceil than from edge to ceil)
                        if(k%2 == 0) { distance_ceils = 0.5;
                        } else { distance_ceils = 1; }
                        Point2D.Double point_ceil = new Point2D.Double();  
                        point_ceil.setLocation(aps_position_list[ap_ix].x + ceil_shifts_inter[k][0], 
                            aps_position_list[ap_ix].y + ceil_shifts_inter[k][1]);
//                        System.out.println(" + Centre of ceil " + k + " = " + point_ceil);
                        // Each ceil contains 4 cubicles                        
                        for(int c = 0; c < 4; c ++) {
                            // Each cubicle contains 4 STAs (randomly placed)
                            for (int s = 0; s < 4; s ++) {
                                double x_sta = 0, y_sta = 0;
                                if(s == 0) {
                                    x_sta = Math.random() * (point_ceil.x - (point_ceil.x-2)) + (point_ceil.x-2);
                                    y_sta = Math.random() * (point_ceil.y - (point_ceil.y-2)) + (point_ceil.y-2);
                                } else if(s == 1) {
                                    x_sta = Math.random() * ((point_ceil.x+2) - point_ceil.x) + point_ceil.x;
                                    y_sta = Math.random() * (point_ceil.y - (point_ceil.y-2)) + (point_ceil.y-2);
                                } else if(s == 2) {
                                    x_sta = Math.random() * (point_ceil.x - (point_ceil.x-2)) + (point_ceil.x-2);
                                    y_sta = Math.random() * ((point_ceil.y+2) - point_ceil.y) + point_ceil.y;
                                } else if(s == 3) {
                                    x_sta = Math.random() * ((point_ceil.x+2) - point_ceil.x) + point_ceil.x;
                                    y_sta = Math.random() * ((point_ceil.y+2) - point_ceil.y) + point_ceil.y;
                                }                               
                                ...
                                stas_position_list[ap_ix][sta_ix] = point_sta;
//                                System.out.println("      * STA" + sta_ix + "'s location = " + stas_position_list[ap_ix][s]);  
                                sta_ix++;                                  
                            }
                        }
                    }                    
    ...
    public static void main(String args[]) throws IOException {
        ...
        String input_path = "./input_constructor/input_template_same_channel_office_scenario_11ax.csv";
        ...
        for (int out_ix = 1; out_ix <= num_outputs; out_ix++) {
            ...
            aps_position_list = new Point2D.Double[num_wlans];
            stas_position_list = new Point2D.Double[num_wlans][num_sta_max/num_wlans];
            
            DefineNodesLocations();            
