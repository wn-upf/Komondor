...
public class AI_challenge_test_sce1 {
...
//                System.out.println("Distance = " + distance);                
            ...
            bss_aux.spatial_reuse_group = -1;
            if (bss_id_sr == i) {
                bss_aux.non_srg_obss_pd = obsspd;
            } else {
...
//                System.out.println("   + STA: " + bss.stas_position_list[n]);
    ...
    static String getCompleteLine(String node_code, int node_type,
        ...
        String line = node_code + CSV_SEPARATOR
                ...
                + cw_adaptation + CSV_SEPARATOR         
                + cont_wind + CSV_SEPARATOR // CW
                + cont_wind_stage + CSV_SEPARATOR // + CSV_SEPARATOR // CW's max stage
                + bss_color + CSV_SEPARATOR
                + spatial_reuse_group + CSV_SEPARATOR
                + non_srg_obss_pd + CSV_SEPARATOR
                + srg_obss_pd;
    ...
    public static void main(String args[]) throws IOException {
        ...
        for (int i = 0; i < num_random_deployments; ++i) {
            for (int b = 0; b < num_bss; ++b) {
                for (int j = 0; j < sensitvity_list.length; ++j) {   
                    // Generate BSSs
                    generate_bss(array_ap_locations, sensitvity_list[j], b);
                    // Specify the output path (file's name)
//                    File theDir = new File("./output/sce" + String.format("%02d", i)
//                        + "_bss" + String.format("%02d", b));
//                    if (!theDir.exists()){
//                        theDir.mkdirs();
//                    }
                    output_path = "./output/input_nodes_sce" + String.format("%02d", i) 
                        + "_bss" + String.format("%02d", b) + "_obsspd" 
                        + String.format("%03d", (int) sensitvity_list[j]) + ".csv";
