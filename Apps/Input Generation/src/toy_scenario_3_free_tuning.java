...
public class toy_scenario_3_free_tuning {

    static Wlan[] wlan_container = null;

    // USER PARAMETERS (input)
    static int num_wlans = 3;          // number of WLANs (M)
    static int central_freq = 5;
    static int channel_bonding_model = 4;
    static int cont_wind_adaptation = 0;         
    static int cont_wind = 16;          // contention window (CW)
    static int cont_wind_stage = 5;     // CW stage     
    static int traffic_model = 99;
    static double traffic_load = 1000;
...
//                    bss_color_input = Integer.parseInt(node_info[25]);
//                    srg_input = Integer.parseInt(node_info[26]);
//                    non_srg_obss_pd_input = Integer.parseInt(node_info[27]);
//                    srg_obss_pd_input = Integer.parseInt(node_info[28]);
//                    
//                    System.out.println(
//                        "Input:"
//                        + "\n- map_width: " + map_width
...
//        double angle = 0;
        ...
        for (int w = 0; w < num_wlans; w++) {
            ...
            num_stas = 1;//ThreadLocalRandom.current().nextInt(num_sta_min, num_sta_max + 1);
            ...
            // Channel allocation
            primary_channel = 0;//ThreadLocalRandom.current().nextInt(0, c_sys_width);
            min_ch_allowed = primary_channel;
            max_ch_allowed = primary_channel;
            
            // IEEE 802.11ax or legacy device
            wlan_80211ax = new Random().nextDouble() <= (1);
            ...
            if (w == 0) {
                point.setLocation(0, 5);
            } else if (w == 1) {
                point.setLocation(10, 5);
            } else if (w == 2) {
                point.setLocation(20, 5);
            }                 
...
//                        System.out.println("WLAN2" + "; " + sensitvity_list[i2] + "; " + tx_power_list[j2]);      
                        for (int i3 = 0; i3 < sensitvity_list.length; i3++) {                        
                            for (int j3 = 0; j3 < tx_power_list.length; j3++) {  
