...
//            System.out.println("  - Setting BSS " + (i+1) + "/" + num_bss);
            ...
            int rnd_ix = 1; //(int)(Math.random()*list_of_ch_width.length);
...
//                System.out.println("Distance = " + distance);            
            ...
            bss_aux.bss_color = i + 1;
            bss_aux.spatial_reuse_group = i + 1;
...
//        System.out.println("File saved in " + csv_filepath_output);
    ...
    public static void main(String args[]) throws IOException {
        ...
        String input_path = "./input_constructor/template_ai_challenge_21_sce1.csv";
        System.out.println("input_path: " + input_path);
        String output_path = "./output/*";

        input_attributes(input_path);
        
        int num_random_deployments = 1000;
                       
        int[] obsspd_list = new int[21]; 
        for (int i = 0; i < 21; ++i) {
            obsspd_list[i] = -82 + i;
        }
        ...
        for (int i = 0; i < num_random_deployments; ++i) { 
            ...
            for (int m = 0; m < num_bss; ++m) {
                distance_condition = 0;
                Point2D.Double ap_location = new Point2D.Double();
                rand_value = new Random().nextDouble();
                x_ap = rand_value * map_width;
                y_ap = rand_value * map_heigth;
                ap_location.setLocation(x_ap, y_ap);
                array_ap_locations[m] = ap_location;  
                if (m > 0) {
                    while (distance_condition == 0) {
                        double distance = getDistance(array_ap_locations[0].x, 
                            array_ap_locations[m].x, array_ap_locations[0].y, array_ap_locations[m].y, 0, 0);
                        if (distance < MIN_DISTANCE_BW_APS) {
                            //System.out.println("Distance = " + distance); 
                            rand_value = new Random().nextDouble();
                            x_ap = rand_value * map_width;
                            y_ap = rand_value * map_heigth;
