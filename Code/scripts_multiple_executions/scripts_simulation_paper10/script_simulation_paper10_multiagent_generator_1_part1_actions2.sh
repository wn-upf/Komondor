...
echo ""
echo "***********************************************************************"
echo "PAPER #10 - Multiagent generator v1 - part 1"
echo "***********************************************************************"
echo ""
...
do
	...
	do
		...
		for MAB_ALG in "explfirst" "epsilon"
			...
			for (( SCENARIO=1; SCENARIO<=25; SCENARIO++ ))
			do

				echo "          * scenario: $SCENARIO/25"
...
./komondor_main ../input/input_paper10/input_nodes/nodes_multiagent_generator_1/input_nodes_n$(printf %d $NUM_AP)_s$(printf %03d $SCENARIO).csv ../input/input_paper10/input_agents/agents_multiagent_generator_1/action_space2/agents_n$(printf %d $NUM_AP)_$(printf %s $REWARD)_$(printf %s $MAB_ALG).csv ../output_part1/script_output.txt sim_n$(printf %d $NUM_AP)_$(printf %s $REWARD)_$(printf %s $MAB_ALG)_s$(printf %03d $SCENARIO)_seed$(printf %d $SEED) 0 1 1 1 1 $SIM_TIME $(printf %d $SEED) >> ../output_part1/logs_console.tx		
